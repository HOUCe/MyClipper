# [python之时间和时间戳转换 - 简书](https://www.jianshu.com/p/2d93ec38ddcf)

[![](https://upload.jianshu.io/users/upload_avatars/7818280/5a83aaf1-ef58-466f-8900-4d99fcc1439e?imageMogr2/auto-orient/strip|imageView2/1/w/96/h/96/format/jpg)](https://www.jianshu.com/u/ec609abeea3a)

0.7852017.09.20 23:23:31字数 298阅读 7,984

## 时间和时间戳的概念

首先我们了解两个基本的概念，时间和时间戳。  
`时间`，我们在本文中更多的讲一个狭义的概念，表示某一时刻的值。  
`时间戳`，unix时间戳是从1970年1月1日（UTC/GMT的午夜）开始所经过的秒数。

## 时间转换为时间戳

-   具体需求  
    将形如`2017-09-20 22:28:54`的时间转换为时间戳
-   实现过程

1.  利用`strptime()`函数将时间转换为时间数组
2.  利用`mktime()`函数将时间数组函数转换为时间戳

-   代码

```

import time

dt = "2017-09-20 22:28:54"


timeArray = time.strptime(dt, "%Y-%m-%d %H%M%S")

timestamp = time.mktime(timeArray)
```

## 重新整理时间格式

-   具体需求  
    将形如`2017-09-20 22:28:54`的时间数据转换为`20170920 22:28`的格式
-   实现过程

1.  利用`strptime()`函数将时间转换为时间数组
2.  利用`mktime()`函数将时间数组函数转换为时间戳

-   代码

```

import time

dt = "2017-09-20 22:28:54"


timeArray = time.strptime(dt, "%Y-%m-%d %H:%M:%S")

dt_new = time.strftime("%Y%m%d %H%M",timeArray)
```

## 时间戳转换为时间

-   具体需求  
    将已知的时间戳转换为具体时间
-   实现过程

1.  利用localtime()函数将时间戳转换为localtime的格式
2.  利用strftime()函数重新格式化时间

-   代码

```

import time

timestamp = 1462451334


time_local = time.localtime(timestamp)

dt = time.strftime("%Y-%m-%d %H:%M:%S",time_local)
```

更多精彩内容，就在简书APP

"小礼物走一走，来简书关注我"

![  ](https://cdn2.jianshu.io/assets/default_avatar/wechat-771371d5741b0c32cd805caeb48ad6c0.png)共1人赞赏

[![  ](https://upload.jianshu.io/users/upload_avatars/7818280/5a83aaf1-ef58-466f-8900-4d99fcc1439e?imageMogr2/auto-orient/strip|imageView2/1/w/100/h/100/format/jpg)](https://www.jianshu.com/u/ec609abeea3a)

### 被以下专题收入，发现更多相似内容

### 推荐阅读[更多精彩内容](https://www.jianshu.com/)

-   python——时间与时间戳之间的转换 对于时间数据，如2016-05-05 20:28:54，有时需要与时间戳进...
    
-   PHP常用函数大全 usleep() 函数延迟代码执行若干微秒。 unpack() 函数从二进制字符串对数据进行解...
    

-   php usleep() 函数延迟代码执行若干微秒。 unpack() 函数从二进制字符串对数据进行解包。 uni...
    
    [![](https://cdn2.jianshu.io/assets/default_avatar/8-a356878e44b45ab268a3b0bbaaadeeb7.jpg)思梦PHP](https://www.jianshu.com/u/cb1fdcaf3d12)阅读 1,713评论 1赞 24
    
-   1.将字符串的时间转换为时间戳方法:a = "2013-10-10 23:40:00"将其转换为时间数组impor...
    
-   1 在我满心憧憬着毕业后能去向往已久的魔都自力更生的时候，我的闺蜜告诉了一...
    
-   我家里有一本书，名字是《后妃辞典》。 这本书记载了从三皇五帝以来他们的皇后和后妃的人生，包括生于何时，嫁于何人，生...
    
    [![](https://cdn2.jianshu.io/assets/default_avatar/10-e691107df16746d4a9f3fe9496fd1848.jpg)亦如是](https://www.jianshu.com/u/a2a7b44bf940)阅读 154评论 0赞 1
    

-   清晨 我独自站在 乡间的小路上 忧郁的眼神中 镶着一株带泪的 黄玫瑰 悄悄地 悄悄地 被它溜到心间 消失了
    

抽奖

![reward](data:image/png;base64,)

Drag to outliner or Upload

Close
