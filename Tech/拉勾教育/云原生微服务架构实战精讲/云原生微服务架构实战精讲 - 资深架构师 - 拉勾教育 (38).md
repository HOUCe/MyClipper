...
下面代码中的 PeerAuthentication 资源只对乘客管理服务启用了双向 TLS 的 STRICT 模式，通过 selector 来指定工作负载。DestinationRule 资源的作用是控制从服务代理发送到乘客管理服务的流量。ISTIO\_MUTUAL 模式的含义是启用双向 TLS，但使用的是 Istio 自动生成的证书。
