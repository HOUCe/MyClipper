...
下面的代码给出了示例应用中地址管理服务的 application.yml 文件内容，该文件只包含了 Spring Data JPA 和 Flyway 相关的配置。配置 spring.datasource.url 的值中使用了环境变量 DB\_HOST、DB\_PORT 和 DB\_NAME，并提供了默认值。在开发环境中，即便没有配置这 3 个环境变量，使用默认值也可以正常工作。
