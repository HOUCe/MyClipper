...
安装完成过后，rollup 这个模块同样会在 node\_modules/.bin 目录中为我们提供一个 CLI 程序，我们就可以通过这个 CLI 去使用 Rollup 打包。具体命令如下：

> P.S. 对于 node\_modules/.bin 目录下的 CLI，我们可以使用 npx 命令或者 yarn 命令直接启动。
...
```
$ npx rollup --config 
$ npx rollup --config rollup.prod.js 
```
...
Rollup 默认只能够按照文件路径的方式加载本地的模块文件，对于 node\_modules 目录中的第三方模块，并不能像 Webpack 一样，直接通过模块名称直接导入。
...
完成以后我们就可以回到代码中直接导入 node\_modules 中的第三方模块了。例如：
