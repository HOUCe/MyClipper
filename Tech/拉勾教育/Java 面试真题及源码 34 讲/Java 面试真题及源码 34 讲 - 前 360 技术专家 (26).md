...
从应用程序启动到采样时 young gc 的次数

YGCT

从应用程序启动到采样时 young gc 的所用的时间（s）

FGC

从应用程序启动到采样时 full gc 的次数

FGCT

从应用程序启动到采样时 full gc 的所用的时间

GCT

从应用程序启动到采样时整个 gc 所用的时间
...
上述信息表示 jhat 启动了一个 http 的服务器端口为 7000 的站点来展示信息，此时我们在浏览器中输入：[http://localhost:7000/](http://localhost:7000/)，会看到如下图所示的信息：
