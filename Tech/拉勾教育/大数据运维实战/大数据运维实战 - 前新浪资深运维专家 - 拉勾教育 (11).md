...
其中，${start\_txid} 表示 edits 的起始事务 id，而 ${end\_txid} 表示 edits 的结束事务 id。正在写入的 edits 文件处于 inprogress 状态，其文件名为 edits\_inprogress\_${start\_txid}，已经写入完成的 edits 处于 finalized 状态，名字就变成了 edits\_${start\_txid}-${end\_txid}。
