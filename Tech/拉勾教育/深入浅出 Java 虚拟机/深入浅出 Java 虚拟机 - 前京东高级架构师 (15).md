...
wget -c https://dl.influxdata.com/influxdb/releases/influxdb-1.7.9\_linux\_amd64.tar.gz
tar xvfz influxdb-1.7.9\_linux\_amd64.tar.gz
...
wget -c https://dl.influxdata.com/telegraf/releases/telegraf-1.13.1-1.x86\_64.rpm
sudo yum localinstall telegraf-1.13.1-1.x86\_64.rpm

Telegraf 通过 jolokia 配置收集数据相对简单，比如下面就是收集堆内存使用状况的一段配置。

\[\[inputs.jolokia2\_agent.metric\]\]
    name  = "jvm"
    field\_prefix = "Memory\_"
...
n -Xloggc:/tmp/logs/gc\_%p.log -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPat
h=/tmp/logs -XX:ErrorFile=/tmp/logs/hs\_error\_pid%p.log -XX:-OmitStackTraceInF
