...
完成上述操作之后，可以请求 [http://localhost:8000/hello/xxx](http://localhost:8000/hello/xxx) ，此时 demo-provider 和 demo-provider 都会分别生成两条 TraceSegment 并通过 Kafka 方式上报。在 Kafka 的命令行 Consumer 中可以看到如下输出：
