...
$ cd /go/src/github.com/wilhelmguo && git clone https:
$ cd gocker
...
> 如果出现 pivotRoot error pivot\_root invalid argument 的报错，可以先执行 unshare -m 命令，然后使用 rm -rf /tmp/busybox/.pivot\_root 命令删除临时文件，再次重试即可。
