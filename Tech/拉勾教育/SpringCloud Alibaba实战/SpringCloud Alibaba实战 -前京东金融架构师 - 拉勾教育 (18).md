...
最后，启动 rm-order，在浏览器地址栏访问 create\_order 接口，看到 create order success，在数据库中也出现对应记录。  
...
TC 向 RM 下达分支事务回滚通知，RM 收到通知做两件事：第一，根据 undo\_log 表生成的反向 SQL，将之前写入的数据撤销；第二，删除 undo\_log 数据。这两步操作保证了 RM 端数据能撤销回之前的状态。
