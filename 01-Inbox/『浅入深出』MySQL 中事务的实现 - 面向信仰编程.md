...
除了锁，另一种实现事务的隔离性的方式就是通过时间戳，使用这种方式实现事务的数据库，例如 PostgreSQL 会为每一条记录保留两个字段；_读时间戳_中包括了所有访问该记录的事务中的最大时间戳，而记录行的_写时间戳_中保存了将记录改到当前值的事务的时间戳。
